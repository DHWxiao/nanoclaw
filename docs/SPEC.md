...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Acai';
...
Messages must start with the trigger pattern (default: `@Acai`):
- `@Acai what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Acai` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Acai what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Acai what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Acai add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Acai remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Acai list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Acai remember I prefer dark mode` | Add to global memory |
...
```
User: @Acai remind me every Monday at 9am to review the weekly metrics
...
```
User: @Acai at 5pm today, send me a summary of today's emails
...
From any group:
- `@Acai list my scheduled tasks` - View tasks for this group
- `@Acai pause task [id]` - Pause a task
- `@Acai resume task [id]` - Resume a paused task
- `@Acai cancel task [id]` - Delete a task

From main channel:
- `@Acai list all tasks` - View tasks from all groups
- `@Acai schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Acai add group "Name"` in main |
